...
GROUP BY dates";
 ...
 //ARRAYS
        $counties = array('Nairobi'=>array('county_target'=>750,'facility_target'=>541),'Mombasa'=>array('county_target'=>750,'facility_target'=>200),'Kilifi'=>array('county_target'=>750,'facility_target'=>200),'Taita Taveta'=>array('county_target'=>750,'facility_target'=>200),'Tana River'=>array('county_target'=>750,'facility_target'=>200),'Lamu'=>array('county_target'=>750,'facility_target'=>200),'Kwale'=>array('county_target'=>750,'facility_target'=>200));
        ...
        $HCW_coast_total = $this->region_trained('Taita Taveta', 10) + $this->region_trained('Lamu', 10) + $this->region_trained('Mombasa', 10)+ $this->region_trained('Kwale', 10)+ $this->region_trained('Kilifi', 10)+ $this->region_trained('Tana River', 10);
        $HCW_coast_ratio = round(($HCW_coast_total / $HCW_coast_target) * 100, 2);
        $HCW_coast_data = $this->facility_type_trained('Taita Taveta', 'HCW', 10) + $this->facility_type_trained('Lamu', 'HCW', 10) + $this->facility_type_trained('Kilifi', 'HCW', 10)+ $this->facility_type_trained('Kwale', 'HCW', 10)+ $this->facility_type_trained('Mombasa', 'HCW', 10)+ $this->facility_type_trained('Tana River', 'HCW', 10);
        ...
        $Facility_coast_total = $this->specific_facilities_trained('Taita Taveta', 10) + $this->specific_facilities_trained('Lamu', 10) + $this->specific_facilities_trained('Kilifi', 10) + $this->specific_facilities_trained('Kwale', 10) + $this->specific_facilities_trained('Mombasa', 10) + $this->specific_facilities_trained('Tana River', 10);
        $Facility_coast_ratio = round(($Facility_coast_total / $Facility_coast_target) * 100, 2);
        $Facility_coast_data = $this->facility_type_trained('Taita Taveta', 'Facility', 10) + $this->facility_type_trained('Lamu', 'Facility', 10) + $this->facility_type_trained('Kwale', 'Facility', 10)+ $this->facility_type_trained('Tana River', 'Facility', 10)+ $this->facility_type_trained('Lamu', 'Facility', 10)+ $this->facility_type_trained('Kilifi', 'Facility', 10);
*/

        foreach($counties as $key=>$county){
            $trained['county']=$this->region_trained($key, 10);
            $trained['facility']=$this->specific_facilities_trained($key, 10);

            $county_data['county'][$key]['name']=$key;
            $county_data['county'][$key]['trained']=$trained['county'];
            $county_data['county'][$key]['ratio']= round(($trained['county'] / $county['county_target']) * 100, 2);

            $county_data['facility'][$key]['name']=$key;
            $county_data['facility'][$key]['trained']=$trained['facility'];
            $county_data['facility'][$key]['ratio']= round(($trained['facility'] / $county['facility_target']) * 100, 2);

            $county_data['facility_type_county'][$key]['type'] = $this->facility_type_trained($key, 'HCW', 10);
            $county_data['facility_type_county'][$key]['target'] = $county['county_target'];
            $county_data['facility_type_county'][$key]['total'] = $trained['county'];

            $county_data['facility_type'][$key]['type'] = $this->facility_type_trained($key, 'Facility', 10);
            $county_data['facility_type'][$key]['target'] = $county['facility_target'];
            $county_data['facility_type'][$key]['total'] = $trained['facility'];
        ...
        switch ($choice) {
            ...
            case 'HCW Progress':
            $county_hcw_data='';
            foreach($county_data['county'] as $county){
                $county_hcw_data.='<div class="summary">'.$county['name'].'
                <div class="progress">
                    <div class="progress-bar" role="progressbar" aria-valuenow="' . $county['ratio'] . '" aria-valuemin="0" aria-valuemax="100" style="width: ' . $county['ratio'] . '%;">
                        ' . $county['ratio'] . '%
                     </div>
                     </div>
                </div>';
                ...
                $data = '<div class="summary"><span class="text">Total HCWs Targeted</span><span class="digit">' . $HCW_target . '</span>
</div>
<div class="summary"><span class="text">Total HCWs Trained</span><span class="digit">' . $HCW_total_trained . '</span>
</div>
<div class="summary">
    <div class="progress">
        <div class="progress-bar" role="progressbar" aria-valuenow="' . $HCW_ratio . '" aria-valuemin="0" aria-valuemax="100" style="width: ' . $HCW_ratio . '%;">
...
';
            ...
            case 'HCW Table':
             $table_data='';
            foreach($county_data['facility_type_county'] as $key=>$county){
                $table_data.='<tr><td rowspan="2">'.$key.'</td><td rowspan="2">' . $county['target'] . '</td><td rowspan="2">' . $county['total'] . '</td><td>Private</td><td>250</td><td>' . $county['type']['Private'] . '</td></tr>
                <tr><td>Public</td><td>500</td><td>' . $county['type']['Public'] . '</td></tr>';
        }
                $data = '<table>
            <thead><th>County</th><th>Target</th><th>Trained</th><th>Facility type</th><th>Target</th><th>Trained</th></thead>
            <tbody>
                 '.$table_data.'
            </tbody>
            <tfoot></tfoot>
            ...
            case 'Facility Progress':
             $facility_hcw_data='';
            foreach($county_data['facility'] as $county){
                $facility_hcw_data.='<div class="summary">'.$county['name'].'
                <div class="progress">
                    <div class="progress-bar" role="progressbar" aria-valuenow="' . $county['ratio'] . '" aria-valuemin="0" aria-valuemax="100" style="width: ' . $county['ratio'] . '%;">
                        ' . $county['ratio'] . '%
                     </div>
...
'.$facility_hcw_data.'
            ...
            case 'Facility Table':
            $table_data='';
            foreach($county_data['facility_type'] as $key=>$county){
                $table_data.='<tr><td rowspan="2">'.$key.'</td><td rowspan="2">' . $county['target'] . '</td><td rowspan="2">' . $county['total'] . '</td><td>Private</td><td>250</td><td>' . $county['type']['Private'] . '</td></tr>
                <tr><td>Public</td><td>500</td><td>' . $county['type']['Public'] . '</td></tr>';
        }
